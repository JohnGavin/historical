...
| `tests/setup.R` sets edition 3 globally (#578) | `tests/setup.R` calls `testthat::local_edition(3, .env = testthat::teardown_env())`, so the root suite (`tests/testthat/`) runs under edition 3 by default. New root test files do **not** need their own `testthat::local_edition(3)` line. Existing files that still declare it individually are harmless (idempotent) and are left alone deliberately — removing them is a separate follow-up, not bundled into this change. |
