...
## 2026-09-08 → 2026-09-10 (session 26 — the data cannot answer the question)
...
- **The validation seal, four leaks deep (#645, #648, #660).** Started as one
  unbounded `OOS` window; ended as a systematic audit. Leaks found, in order of
  discovery — each by accident while fixing the previous one:
  1. **#645** — `mf_metrics`/`ev_metrics` computed `dates >= 2010-01-01` with no
     upper bound, so the sealed Validation partition was recomputed and
     published on every `tar_make()`. Bounded at `test_end` (PR #649, gate S11).
  2. **#648** — `slice_portfolio()` computed an explicit `Validation` slice for
     7 strategies. The dashboard filtered it from *display*, but the rule
     constrains *computation*. Stripped at `all_metrics` assembly (PR #659,
     gate S14). Found while measuring #635.
  3. **#655** — the tracked digest snapshot parquet carries Validation rows for
     5 strategies **into git history**, and a full `tar_make()` does not refresh
     it. Open.
  4. **#660** — `docs/stock-backtest.qmd` **published** Validation Sharpe/CAGR/
     max-DD in prose, and line 286 drew a strategy conclusion from it
     (*"generalisation to sealed Validation has not held"*). Removed, with lines
     286/542 rewritten to rest only on Training/Testing evidence (PR #662, gate
     S15). The fix turned out to be ~2× the size the issue described: six
     metrics tables were also rendering raw Validation rows unfiltered.
- **#660 partition re-cut — four tiers (PR #666).** No re-slicing of existing
  data yields a clean window: every month from 2024-01 to the data boundary sat
  inside the published block. So: Training ≤2019-12-31, Testing →2023-12-31,
  **Holdout** 2024-01-01→2026-04-30 (*observed, never called sealed*),
  Validation 2026-05-01→ (genuinely untouched, empty at merge time). Holdout
  surfaced on the leaderboard by adding the period to all 7 source-metrics
  targets.
- **#641 — the PSO portfolio was missing every March.** Root cause was not the
  join: `stk_drif_features()` built its 21-trading-day lookback from a single
  preceding calendar month, and February never supplies 21 days, so `c20`/`c21`
  were `NA` and `predict.glmnet()` propagated `NA` across the whole row. Wiped
  March in 14/17 years outright; the 3 leap-year survivors fell below the
  min-stocks gate (PR #652). Separately, the 4-way `inner_join` chain in
  `port_returns` turned one constituent's gap into a hole for all four —
  replaced with a bounded spine + weight renormalisation (PR #651, gates
  S12/S13).
- **#640 — `bt.metric` units (PR #650).** `metric_unit` was recorded but neither
  validated nor converted. Constrained to an enum, aborts on absent/unknown,
  canonicalises to fractions on read. Audit found **100% NULL** across ~256 rows
  — a live inconsistency, not a latent one; regenerated by the pipeline run.
- **#669 — `adjusted` vs `adjusted_close` (PR #670).** `hd_ohlcv()` returns
  `adjusted_close`; 12 references across 12 files selected `adjusted`. Broken
  since 2026-05-28 (#325/PR #332) — **~10 weeks** — and invisible because the
  targets were cached and never invalidated. `olmar_prices` was the same bug
  wearing a disguise: a per-ticker `tryCatch` swallowed the schema error and
  emitted a misleading *"check network access"* message. A
  `.hd_assert_price_schema()` guard now fails at the access boundary.
- **#654 — `verify.sh` now *asserts* the skip count (PR #658).** It printed a
  warning on drift but never set `OVERALL_STATUS`, so it always exited 0. The
  discipline was documented, printed, and unenforced.
- **#635 — measured and decided.** `sigma_target` = **11.5%** (the level at
  which total implied gross equals total current gross, so the control
  re-levers nothing on day one), gross backstop **3.5×**, book realised vol
  **6.57%**. The ~5pp gap between book and per-strategy is the diversification
  benefit, not an inconsistency.
- **Two rules.** `fail-loud-not-null` (PR #647) — an unrecognised value must
  abort, not coerce to `NA`/`NULL`/a dropped row; every instance gets a *gate*,
  not just a test. `backtest-partitions` (PR #661, extended in PR #666) — the
  seal now constrains **computation, storage, display and reasoning**, and its
  `paths:` frontmatter was widened because the old globs matched only files that
  *compute* partitions, so the rule never loaded for the file that published
  them.
- **Replied to an external contributor on #624**, filing #663/#664/#665 rather
  than adopting the suggestion directly.

### Failed Approaches

- **#635 Option B as written** (anchor on the book's realised vol) — blocked:
  the only whole-book proxy was PSO Optimal, whose series #641 had truncated by
  18%. Recovered by anchoring on gross-budget neutrality instead, then
  re-verified against the repaired data (11.52% → 11.60%, decision unchanged).
- **2024-01-01 as a "clean" validation cut** — rejected. All 28 months sat
  inside the published aggregate, so it yields a *smaller fully-observed*
  window, not a cleaner one. Only data past the boundary (2026-05-01) is
  genuinely untouched.
- **2026-09-01 as the forward cut** — too conservative; wasted four months.
  2026-05-01 is the first month past the data boundary.
- **Filing 8 `inner_join` issues** — the audit showed 6 were benign by
  construction or fed nothing published. Only 2 warranted action, plus a 3rd
  the original sweep had missed (#656). Filing all 8 would have buried them.
- **#654 was filed on a wrong premise** — I claimed the docs said 5 skips;
  `main` already said 15. I had read a stale worktree copy. Three agents
  independently repeated the figure without anyone re-reading the source. The
  underlying defect was real; the stated reason was not.
- **Two dispatch errors, both mine.** Two agents were given the same gate label
  (S11) because only one was warned off it. And an agent hit the push guard on
  a branch still checked out in another worktree, reaching for
  `AGENT_PUSH_OK=1` without that being in its scope block.

### Accuracy / Metrics

- QA gates: **2 → 7** (S9–S15), all passing on real data
- `stk_drif_portfolio`: 129 → **195** rows; **March rows 0 → 17**
- `port_returns`: 128 → **195** rows
- PSO Optimal: 128 → **195** months; **max_dd −0.099 → −0.212** (had been
  understated by more than half)
- Registry rows with NULL `metric_unit`: ~256 → **0**
- Root test suite: 326 → **397**
- Final pipeline: **59 built, 703 skipped, 2m 35s, 0 errors**

### Known Limitations

- **#667 — two strategies have unbounded `Testing` windows.** Risk State
  (`R/plan_risk_state.R:301-304`) and Avoid Worst
  (`R/plan_avoid_worst.R:459`) use `date >= oos` with no upper bound, under the
  *canonical* `Testing` label. Found by a natural experiment: moving `test_end`
  a year made every compliant strategy gain 12 months (or 250 trading days);
  **exactly these two did not move.** S11 never inspected them because its
  scope is a hardcoded pair (`mf_metrics`, `ev_metrics`) — the same
  scope-drawn-around-known-instances failure as the rule's `paths:` glob.
- **σ = 11.5% will need re-deriving** once #667 and #665 land — both change the
  inputs it was measured from.
- **#665 — financing is modelled on neither side consistently.** 4 of 17
  strategies carry a borrow cost; **none** earns securities-lending income. The
  omissions bias in opposite directions, so this is inconsistency rather than
  conservatism. Three strategies (Mom Pre-Peak/Post-Peak/12-2) run a 100% short
  leg with no borrow cost — returns overstated by an unquantified amount.
- **#655 — the tracked digest snapshot is 2 months stale and `tar_make()` does
  not refresh it.** A tracked artefact a full run silently skips is worse than a
  merely stale one.
- **#669's real lesson:** targets were unbuildable for ~10 weeks while the
  pipeline reported green, because caching hid them. **"Green" means "nothing
  changed", not "everything works."** A periodic full-rebuild check would have
  caught it in a day; not built.
- **#668 — seven instances of one defect shape** (a unit or vocabulary recorded
  but never validated or converted), every one found by accident and none by a
  gate. Escalation agreed: implement `data/glossary.yaml` +
  `data/entity_resolution.yaml` (already prescribed by a global rule, never
  built here) and one registry-driven gate replacing the per-instance pattern.
  Parked pending the partition work settling.
- Automated data refreshes on 2026-08-08 may push data past 2026-04-30, in which
  case the Validation window starts accruing — worth checking before the next
  `evaluate_validation.R` run.
- Open: #655, #656, #657, #663, #664, #665, #667, #668.
