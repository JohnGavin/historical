...
Node colours: Factors (blue) | Macro (gold) | Regime (orange) | Signals (purple) | Returns (green) | Outcomes (red) | Structural (grey). Yellow dashed = violated implication. Green = tested, holds. Source: [plan_causal_graph.R](https://github.com/JohnGavin/historical/blob/main/R/plan_causal_graph.R).
...
DRIF (Dynamic Rotation Into Factors) selects the best-performing FF factor each month. Loads on HML, SMB, Mom, RMW. Yellow dashed edge: VIX affects HML in crises (r=-0.17). Source: [plan_drif.R](https://github.com/JohnGavin/historical/blob/main/R/plan_drif.R).
...
Factor MAX selects the top-momentum factor. Premium has decayed >50% (#73). Source: [plan_factormax.R](https://github.com/JohnGavin/historical/blob/main/R/plan_factormax.R).
...
LTR (LambdaMART) uses XGBoost for cross-sectional stock ranking. Loads on Mom + SMB. Rate regime feeds into signal. Source: [plan_ltr_momentum.R](https://github.com/JohnGavin/historical/blob/main/R/plan_ltr_momentum.R).
