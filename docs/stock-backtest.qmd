...
| Finding | Evidence |
|---------|----------|
| Stock MAX has the highest raw CAGR (`r { m <- safe_tar_read("stk_max_metrics"); v <- if(!is.null(m)) m$cagr[m$period=="Full Period"] else numeric(0); if(length(v)==1L) paste0(round(v*100,1),"%") else "N/A" }`) | But with `r { m <- safe_tar_read("stk_max_metrics"); v <- if(!is.null(m)) m$vol[m$period=="Full Period"] else numeric(0); if(length(v)==1L) paste0(round(v*100,0),"%") else "N/A" }` vol — risk-adjusted Sharpe is modest (`r { m <- safe_tar_read("stk_max_metrics"); v <- if(!is.null(m)) m$sharpe[m$period=="Full Period"] else numeric(0); if(length(v)==1L) round(v,2) else "N/A" }`) |
...
| Finding | Evidence |
|---------|----------|
| Stock-level strategies have 2-5x higher vol than factor-level | `r { m <- safe_tar_read("stk_max_metrics"); v <- if(!is.null(m)) m$vol[m$period=="Full Period"] else numeric(0); if(length(v)==1L) paste0(round(v*100,0),"%") else "N/A" }` vol (stock MAX) vs `r { m <- safe_tar_read("fm_metrics"); v <- if(!is.null(m)) m$vol[m$period=="Full Period"] else numeric(0); if(length(v)==1L) paste0(round(v*100,0),"%") else "N/A" }` (factor MAX) |
...
if (!is.null(metrics)) {
  display <- metrics |>
    ...
    mutate(across(c(cagr, vol, max_dd), ~ paste0(round(. * 100, 1), "%")),
           sharpe = round(sharpe, 2),
           avg_long = round(avg_long), avg_short = round(avg_short))
  hd_dt_wide(display, "XGBoost DRIF metrics. Underperforms elastic net across all periods. Negative Sharpe confirms monotonic constraints are too restrictive for cross-sectional stock returns.")
}
```
