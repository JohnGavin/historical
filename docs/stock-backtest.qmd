...
if (!is.null(fi)) {
  ...
  hd_dt(fi_display, "Top 15 features by XGBoost gain. Trained on a single full-training-period model (not the rolling expanding window). Gain concentration in one feature (c13 = 48%) is a red flag for model instability, not a meaningful finding.")
}
```
...
The apparent dominance of a single feature (c13 ≈ 48% of gain) should NOT be interpreted as "day 13 of a month is special." This is an artifact of:

1. **Monotonic constraints** — with all 42 features forced monotonically increasing, XGBoost must route most splits through whichever feature best separates the training data. This creates artificial concentration.
2. **Single model snapshot** — this is ONE model trained on ALL training data. The rolling expanding-window models (used for actual predictions) likely have different dominant features each month.
3. **Shallow trees (depth=3)** — with only 3 splits per tree, the first split dominates gain. A deeper tree would distribute importance more evenly.

The correct interpretation: XGBoost with monotonic constraints is **not well-suited** for this cross-sectional prediction task. The constraints are too restrictive and the feature importance is unreliable.
:::
