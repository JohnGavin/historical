...
#
# Entity glossary (#668) MUST be sourced before plan_partitions.R:
# PERIOD_LABELS_ALLOWED there is derived from load_glossary() at source time
# (mirrors the ordering in docs/_targets.R).
glossary_path <- if (file.exists("../R/glossary.R")) "../R/glossary.R" else
  file.path(here::here(), "R/glossary.R")
source(glossary_path)
...
if (!is.null(lb)) {
  ...
  #
  # sharpe_haircut_pct (#490 Gap 1) is the complementary Harvey-Liu
  # multiple-testing haircut (hd_sharpe_haircut(), method = "bhy") --
  # joined the same way, alongside dsr_haircut_pct, so the tooltip shows
  # both the DSR view and the named-correction view side by side.
  ...
  if (!is.null(strat_ds) && "sharpe_haircut_pct" %in% names(strat_ds)) {
    full <- full |> left_join(strat_ds |> select(strategy, sharpe_haircut_pct), by = "strategy")
  }
  ...
  rigour_badge <- function(strategy, deflated, dsr_p, keff, kraw, sr, haircut,
    ...
    #
    # mt_haircut (#490 Gap 1) is the complementary Harvey-Liu BHY
    # multiple-testing haircut (hd_sharpe_haircut()) -- informational only,
    # appended to the tooltip alongside the DSR haircut. It does NOT
    # participate in the 3/3 coverage count above (that stays scoped to
    # deflated Sharpe / DSR p-value / K_eff, per QA gate S21, #728 item 4).
    ...
    if (present == 3L) {
      ...
      mt_txt <- if (is.na(mt_haircut)) "" else sprintf(" Harvey-Liu BHY haircut: %.1f%%.", mt_haircut)
      return(list(
        html = sprintf(
          "<span class='rig-badge computed' title='Deflated Sharpe: %.2f (naive %.2f%s). DSR p-value: %.3f. K_eff = %.1f of %s raw.%s'>&#10003; 3/3</span>",
          deflated, sr, haircut_txt, dsr_p, keff, kraw, mt_txt
        ),
        rank = 1L
      ))
    }
  ...
  # target" -- infinite-horizon, drifted-Brownian-motion approximation, see
  # the plan_leaderboard.R comment above `fp_breach_prob_20pct` for the full
  # derivation and its documented scope limits. This column INFORMS; it does
  # NOT rank -- deliberately no hidden `.rank` sort column and a single
  # neutral, muted style regardless of value (per #586's "Proposed shape":
  # "flagged, never removed" from the primary board, and never averaged into
  # the primary ordering).
  fp_badge <- function(prob) {
    if (is.na(prob)) {
      return("<span class='fp-badge' title='Not computed -- see plan_leaderboard.R fp_breach_prob_20pct comment for the NA paths (missing cagr/vol/obs_ann_factor, or hd_first_passage() itself failing).'>not computed</span>")
    }
    sprintf(
      paste0(
        "<span class='fp-badge' title='Probability of hitting a -20%% drawdown ",
        "floor before a +20%% profit target, from a drifted-Brownian-motion ",
        "approximation (historicaldata::hd_first_passage()). Infinite horizon ",
        "(no time limit) and assumes Gaussian iid returns -- real fat-tailed ",
        "paths breach MORE often than this implies. Informational only: does ",
        "NOT affect this table&#39;s ranking. See issue #586.'>%.0f%%</span>"
      ),
      prob * 100
    )
  }
  ...
  fp_html <- if ("fp_breach_prob_20pct" %in% names(full)) {
    ...
    rep(fp_badge(NA_real_), nrow(full))
  }
  ...
  metrics_table <- full |>
    mutate(
      ...
      # SECONDARY, de-emphasised column (#586 G1) -- see fp_badge() above.
      # Deliberately placed LAST: it informs, it never ranks (no hidden
      # `.rank` sort column, unlike every badge column before it).
      `Breach Risk (±20% DD)` = fp_html
    ) |>
...
/* fp-badge (#586 G1): deliberately a SINGLE, neutral, muted style
   regardless of value -- this column informs, it never ranks, so unlike
