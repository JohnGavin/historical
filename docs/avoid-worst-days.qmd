...
::: {.fig-caption}
Rolling 1-year net benefit (in percentage points) of perfectly avoiding the 10 worst days. Spikes during crisis periods when the worst days are most extreme.
:::
...
asym <- safe_tar_read("aw_asymmetry_table")
if (!is.null(asym)) {
  ff_asym <- asym |> filter(ticker == "Mkt-RF")
  if (nrow(ff_asym) > 0) {
    DT::datatable(ff_asym,
      caption = htmltools::tags$caption(
        style = "caption-side: top; text-align: left; font-weight: bold; color: #e0e0e0;",
        "Fama-French Mkt-RF (1926+): same asymmetry pattern over nearly a century of data."
      ),
      rownames = FALSE,
      options = list(pageLength = 15, scrollX = TRUE, dom = "t")
    )
