...
3. **Predict** next month's return using elastic net (`glmnet::cv.glmnet`, alpha = 0.5) trained on expanding window
...
- **Elastic net only:** We use `glmnet::cv.glmnet` (alpha = 0.5) throughout; there is no PCA-OLS fallback path in the current implementation
