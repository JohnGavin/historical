...
hd_ticker_groups <- function() {
  groups <- list(
    ...
    list("DeFi Altcoins",
         ...
         c("XRP", "ADA", "DOGE", "DOT")),
    list("Defense First",
         "Macro rotation: 4 hedges covering deflation, gold, commodities, USD",
         c("TLT", "GLD", "DBC", "UUP")),
    list("Macro Hedge ETFs",
         "Extended macro hedge universe for backtesting",
         c("TLT", "GLD", "DBC", "UUP", "IEF", "SHY", "TIP", "IAU",
           "PDBC", "BIL", "AGG", "EFA", "EEM")),
    list("FTSE 100 ETFs",
         "ETFs tracking the FTSE 100 index on LSE",
         c("ISF.L", "VUKE.L", "CUKX.L")),
    list("FTSE 250 ETFs",
         "ETFs tracking the FTSE 250 mid-cap index on LSE",
         c("MIDD.L")),
    list("UK Bond ETFs",
