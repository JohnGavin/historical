# hd_commodity_mr_conditioning_universe: unmapped series_id aborts loudly (fail-loud-not-null)

    Code
      hd_commodity_mr_conditioning_universe(tbl)
    Condition
      Error in `hd_commodity_mr_conditioning_universe()`:
      x 1 series_id in `returns_tbl` is not in the CMR exposure map.
      i Unmapped: "NOT_A_REAL_TICKER".
      i Add "NOT_A_REAL_TICKER" to .HD_CMR_EXPOSURE_MAP (packages/historicaldata/R/commodities_mean_reversion.R) with its underlying_exposure, keep, and conditioning decision before it can be classified -- see #751 item 1.

# hd_commodity_mr_conditioning_universe: input validation -- not a data frame / missing series_id

    Code
      hd_commodity_mr_conditioning_universe(list(a = 1))
    Condition
      Error in `hd_commodity_mr_conditioning_universe()`:
