...
#
# .md pages (historical#0b412a9c follow-up): the "render every page" set
# also includes any docs/*.md file that ALREADY has a sibling .html (i.e. it
# is a rendered site page, not a repo-internal doc like docs/DASHBOARDS.md
# that has no .html and was never part of the site). Without this,
# docs/api-historicaldata.md -- regenerated by scripts/regen_api_context.sh
# on every packages/historicaldata/R export change -- never gets
# re-rendered by --render-all, so its .html can carry both stale content
# AND a stale project-level <meta name="color-scheme"> once that became
# page-level instead of per-qmd. Same reasoning applies to
# t-lang-lessons.md/etf-replication-notes.md/MERMAID_LESSONS.md, which are
# static but still needed the new project-level meta/css baked in at least
# once. `quarto render docs/X.md` renders identically to `quarto render
# docs/X.qmd` -- Quarto treats executable/non-executable .md the same as
# .qmd for a single-file render.
...
#      patterns Step 4 greps for, OR Step 4b's site-wide
#      check_dashboard_color_scheme.sh found a Quarto-rendered page missing
#      its color-scheme signal (historical#0b412a9c follow-up). Deliberately
#      a DIFFERENT code from exit 1: a render failure is not a build
#      failure -- the pipeline and its data are fine, only the republish
#      step for one or more pages is not.
...
# comment above).
# ---------------------------------------------------------------------------
# Helper for the "render every page" cases below (--render-all, and the
# fallback when Step 3's completion marker is missing): docs/*.qmd plus any
# docs/*.md that ALREADY has a sibling .html. The .html-exists check is what
# excludes repo-internal markdown -- e.g. docs/DASHBOARDS.md -- that was
# never part of the rendered site and has no .html to compare against; see
# the --render-all header comment above for the api-historicaldata.md case
# this closes.
all_renderable_pages() {
  find "$REPO_ROOT/docs" -maxdepth 1 -name '*.qmd' -exec basename {} \;
  # NOTE: the `if` form below is required, not stylistic -- under this
  # script's `set -euo pipefail`, a bare `[[ -f ... ]] && echo "$m"` makes
  # the whole script exit the moment a .md file WITHOUT a sibling .html is
  # seen (the failing `[[ ]]` becomes the last command's exit status,
  # tripping errexit), silently truncating this function's output and
  # everything after it. Caught by manual testing before commit -- there is
  # no test harness coverage for this helper.
  find "$REPO_ROOT/docs" -maxdepth 1 -name '*.md' -exec basename {} \; | while IFS= read -r m; do
    if [[ -f "$REPO_ROOT/docs/${m%.md}.html" ]]; then
      echo "$m"
    fi
  done
}
...
if [[ "$RENDER" == "true" ]]; then
  ...
  if [[ "$TAR_MAKE_STATUS" -ne 0 ]] || [[ "$CHECK_STATUS" -ne 0 ]] || [[ "$CROSSCHECK_STATUS" -ne 0 ]] || [[ "$PKGSTALE_STATUS" -ne 0 ]]; then
    ...
    if [[ "$RENDER_ALL" == "true" ]]; then
      echo "--render-all requested (#740) -- rendering every docs/*.qmd page, plus any"
      echo "docs/*.md page that already has a sibling .html, regardless of staleness."
      STALE_PAGES="$(all_renderable_pages | sort -u)"
      ...
      echo "!!! (+ already-published docs/*.md) pages instead of only the stale ones --"
      echo "!!! more expensive, but never silently renders nothing when the true stale"
      echo "!!! set is unknown."
      STALE_PAGES="$(all_renderable_pages | sort -u)"
    fi
    ...
    if [[ -z "$STALE_PAGES" ]]; then
      ...
      while IFS= read -r page; do
        [[ -z "$page" ]] && continue
        qmd_path="docs/$page"
        # page is .qmd for every page Step 3 flags, and either .qmd or .md
        # for the "render every page" cases above (all_renderable_pages) --
        # strip whichever extension is actually present rather than
        # assuming .qmd, or an .md page's html_path would come out as
        # "foo.md.html" (never matching the real "foo.html").
        case "$page" in
          *.qmd) html_path="docs/${page%.qmd}.html" ;;
          *.md)  html_path="docs/${page%.md}.html" ;;
    ...
    # Step 4b: color-scheme signal check (historical#0b412a9c follow-up).
    # check_dashboard_color_scheme.sh takes a DIRECTORY and greps every
    # *.html under it -- a whole-site invariant, not a per-page one. It used
    # to run from docs/_quarto.yml's `project: post-render:`, which is
    # correct for a single whole-project `quarto render` (fires once, after
    # everything is done) but WRONG for this script, which renders pages one
    # at a time via `quarto render docs/X.qmd` in the loop above: Quarto
    # fires `post-render:` after EVERY invocation, so the check re-scanned
    # the whole directory after each single-page render and failed on
    # siblings that simply had not been re-rendered yet in this run.
    # Observed: a --render-all run failing on every one of 15 pages, in
    # rendering order, purely from this ordering bug -- not from any actual
    # missing signal. Running it exactly ONCE here, after the per-page loop
    # above has finished (whether it rendered 0, some, or all pages), means
    # every page this invocation was going to (re-)render already has, so a
    # failure now reflects a real missing signal rather than mid-render
    # directory state. See docs/_quarto.yml's post-render: comment for the
    # matching note on the removed entry.
    echo "--- Step 4b: color-scheme signal check (accessibility rule Part 2 Clause 0) ---"
    set +e
    CS_OUT="$(bash /Users/johngavin/docs_gh/llm/.claude/scripts/check_dashboard_color_scheme.sh "$REPO_ROOT/docs" 2>&1)"
    CS_STATUS=$?
    set -e
    echo "$CS_OUT"
