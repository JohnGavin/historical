...
~10,490 by 1998, **declines** to a trough of ~7,466 (2012), then rises again
to a new high of 10,907 by 2024.

> **Orchestrator correction (2026-09-12).** The original draft of this
> section read the rise-decline-rise shape as "the signature of continuous
> entry and exit". That claim does not survive checking and is withdrawn.
>
> The recovery to **10,907 in 2024 -- above the 1998 peak of 10,490 --
> contradicts the documented US listing decline.** Doidge, Karolyi & Stulz's
> "listing gap" has US listed firms falling from roughly 7,500 in 1997 to
> about half that by the late 2010s, with no recovery. A US-common-stock
> panel does not look like this.
>
> The likeliest reading is that OSAP's `permno` universe is **all CRSP
> securities** -- ETFs, ADRs, REITs, closed-end funds included -- not the
> common-stock subset. That is not a defect in OSAP, but it means the panel
> shape is **not** corroboration of delisting inclusion, and should not be
> cited as such.
>
> **P1 does not depend on it.** The 86.7% exit rate is direct evidence and
> stands on its own. What changes is a caveat, not the verdict -- see the
> universe-breadth limitation below.

**Universe-breadth limitation (added 2026-09-12).** The firm-level file has
exactly 211 columns: `permno`, `yyyymm`, and 209 signals. There is **no
`shrcd`, no `exchcd`, no `siccd`, no ticker and no CUSIP**, so the panel
**cannot be filtered to US common stocks**. Any consumer sorting over it is
sorting over an unfiltered CRSP-wide universe. Zadeh (the motivating claim in
#857) studies "US stocks"; that is a different population, and a replication
built on this panel has to say so rather than sort over everything and call
it the same test.
