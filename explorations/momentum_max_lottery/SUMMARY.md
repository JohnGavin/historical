...
### The ranking inverts depending on which statistic you read

Reading the three long/short strategies by Sharpe alone gives the exact opposite ordering to every other statistic:

| Strategy (gross) | Sharpe | CAGR | Max DD | Skew |
|---|---:|---:|---:|---:|
| mom_plain | 0.163 | +1.27% | −64.5% | −1.56 |
| mom_lottery | **0.221** (best) | **−0.69%** (worst) | **−96.6%** (worst) | **−5.66** (worst) |
| mom_nonlottery | 0.200 | **+1.73%** (best) | **−62.9%** (best) | **+0.14** (best) |

`mom_lottery` ranks first on Sharpe and last on all three of CAGR, drawdown and skew. The mechanism is ordinary — a positive arithmetic mean (+4.86%) with 22.0% volatility and −5.66 skew produces a respectable Sharpe and a *negative* compounded return, because the drawdown that takes the strategy down 96.6% is not recoverable. Sharpe is computed on arithmetic monthly returns and is blind to it.

This matters for two separate reasons.

**First, as a caution about the comparison in this exploration.** Had `mom_lottery` vs `mom_nonlottery` been judged on Sharpe alone — as the brief's "does the filter beat the control" question invites — the filtered strategy would have looked like the winner, gross and net. On the numbers that determine whether an investor ends up with more money, it is decisively the loser. The detection-power verdict already says neither gap is real; this says that even the *sign* of the nominal gap depends on which statistic is consulted.

**Second, it is weak but real corroboration of Zadeh's *mechanism*, even though the tradeable claim failed.** Klement's summary asserts lottery stocks "are likely to be one of the sources of momentum crashes". In our data the momentum strategy built on high-MAX names has skew −5.66 and a 96.6% drawdown, against +0.14 and 62.9% for the identical strategy built on low-MAX names. So filtering momentum *toward* lottery names concentrated crash risk here, sharply, in a panel that by construction excludes the worst crashes.

That is a different claim from the one that failed. The −14.8% loser-leg return is untestable on this data (§d). The proposition that lottery-ness concentrates momentum crash risk shows up anyway — in the third and fourth moments rather than the mean, which is where a survivorship-biased panel would still be expected to show it, since the surviving lottery names retain the shape of the distribution even after its worst realisations have been removed. This is suggestive, not evidence: it is one sample, the effect is measured on a strategy whose mean is indistinguishable from zero, and no significance test has been run on a skew or drawdown difference. It is recorded as a hypothesis worth testing properly on delisting-inclusive data, not as a result.
...
- `results/stock_month_signals.parquet` — the full per-ticker, per-formation-month signal table (momentum, MAX, terciles, holding return), ~198,761 rows. **Not committed.** At 9.4MB it is ~2000x the committed footprint of any sibling exploration's results, and it is fully regenerable by re-running `run.R`. Re-run the script if you need it.
